...
screening simulation - radiologist reliance sensitivity analysis
...
def run_single_simulation_alpha_analysis(seed: int) -> pd.DataFrame:
    """
    ...
    try:
        ...
        for radiologist_alpha in alpha_arr:

            print(f'alpha: {radiologist_alpha}')
            ...
            stats_df.loc[:, 'alpha'] = radiologist_alpha
            alpha_df_list.append(stats_df)

        alpha_df = pd.concat(alpha_df_list, ignore_index=True)
        return alpha_df
...
def parallel_simulations_alpha_analysis(
    n_sims: int = 100,
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=n_cores) as executor:
        # Submit all tasks
        future_to_seed = {
            executor.submit(run_single_simulation_alpha_analysis, seed.generate_state(1)[0]): seed
            for seed in seeds
        }
...
if __name__ == '__main__':
    ...
    results = parallel_simulations_alpha_analysis(n_sims=n_sims)
    end_time = time.time()
