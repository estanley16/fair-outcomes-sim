...
screening simulation - treatment delay sensitivity analysis
...
delay_loc_arr = [-3.8, -1.9, -0.7, 0.5, 2.2]
# ------------------------------------------------------------------------------------ #

def run_single_simulation_delay_analysis(seed: int) -> pd.DataFrame:
    """
    ...
    try:
        ...
        for delay_fxn_loc in delay_loc_arr:

            print(f'delay loc parameter: {delay_fxn_loc}')
            ...
            stats_df.loc[:, 'delay_loc'] = delay_fxn_loc
            delay_df_list.append(stats_df)

        delay_df = pd.concat(delay_df_list, ignore_index=True)
        return delay_df
...
def parallel_simulations_delay_analysis(
    n_sims: int = 100,
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=n_cores) as executor:
        # Submit all tasks
        future_to_seed = {
            executor.submit(run_single_simulation_delay_analysis, seed.generate_state(1)[0]): seed
            for seed in seeds
        }
...
if __name__ == '__main__':
    ...
    results = parallel_simulations_delay_analysis(n_sims=n_sims)
    end_time = time.time()
