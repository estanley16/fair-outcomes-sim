...
screening simulation - AI performance disparity sensitivity analysis
...
#define baseline probability of false positive for unpriv group --> priv group FPR increases based on performance disparity
p_FP_AI_unpriv = 0.05

#define baseline prob of TPs for risk scores 1 and 4 for priv group --> unpriv group TPR decreases based on performance disparity
p_TP_AI_priv = np.array([0.75, 0.95])


# ------------------------------------------------------------------------------------ #

def run_single_simulation_performance_disp_analysis(seed: int) -> pd.DataFrame:
    """
    ...
    #initialize dataframes to collect outcome metrics - MR disparity, MR ratio, SEI disparity, change in SEI disparity
    ...
    for dFPR in dFPR_list:
        ...
        p_FP_AI_priv = p_FP_AI_unpriv + dFPR #priv FPR increases based on delta

        for dTPR in dTPR_list:


            p_TP_AI_unpriv = p_TP_AI_priv - dTPR #unpriv group TPR dereases based on delta
    ...
    return dMR_df, MR_ratio_df, dSEI_df, pc_dSEI_df, dSEI_med_df, pc_dSEI_med_df




def parallel_simulations_performance_disp_analysis(
    n_sims: int = 100,
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=n_cores) as executor:
        # Submit all tasks
        future_to_seed = {
            executor.submit(run_single_simulation_performance_disp_analysis, seed.generate_state(1)[0]): seed
            for seed in seeds
        }
        ...
        for future in concurrent.futures.as_completed(future_to_seed, timeout=timeout):
            ...
            dMR, MR_ratio, dSEI, pc_dSEI, dSEI_med, pc_dSEI_med = future.result() #returns from run_single_simulation()
    ...
    return dMR_results, MR_ratio_results, dSEI_results, pc_dSEI_results, dSEI_med_results, pc_dSEI_med_results
...
if __name__ == '__main__':
    ...
    dMR_results, MR_ratio_results, dSEI_results, pc_dSEI_results, dSEI_med_results, pc_dSEI_med_results = parallel_simulations_performance_disp_analysis(n_sims=n_sims)
    end_time = time.time()
