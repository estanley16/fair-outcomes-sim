...
screening simulation - screening sensitivity analysis
...
screening_loc_arr = [-10, -7, -5, -3, 0]

# ------------------------------------------------------------------------------------ #


def run_single_simulation_screening_analysis(seed: int) -> pd.DataFrame:
    """
    ...
    try:
        ...
        for screening_fxn_loc in screening_loc_arr:

            print(f'screening loc parameter: {screening_fxn_loc}')
            ...
            stats_df = sim.run()
            stats_df.loc[:, 'screening_loc'] = screening_fxn_loc
            screening_df_list.append(stats_df)

        screening_df = pd.concat(screening_df_list, ignore_index=True)
        return screening_df
...
def parallel_simulations_screening_analysis(
    n_sims: int = 100,
    ...
    with concurrent.futures.ProcessPoolExecutor(max_workers=n_cores) as executor:
        # Submit all tasks
        future_to_seed = {
            executor.submit(run_single_simulation_screening_analysis, seed.generate_state(1)[0]): seed
            for seed in seeds
        }
...
if __name__ == '__main__':
    ...
    results = parallel_simulations_screening_analysis(n_sims=n_sims)
    end_time = time.time()
